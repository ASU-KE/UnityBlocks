...
class UnityBlocks_Form {
	...
	public function default_success_text() {
		 ...
		 */
		return (string) apply_filters( 'unityblocks_form_sent_notice', __( 'Your message was sent:', 'unityblocks' ) );
	}
	...
	public function register_settings() {
		register_setting(
			'unityblocks_google_recaptcha_site_key',
			'unityblocks_google_recaptcha_site_key',
			array(
				'type'              => 'string',
				'description'       => __( 'Google reCaptcha site key for form spam protection', 'unityblocks' ),
		...
		register_setting(
			'unityblocks_google_recaptcha_secret_key',
			'unityblocks_google_recaptcha_secret_key',
			array(
				'type'              => 'string',
				'description'       => __( 'Google reCaptcha secret key for form spam protection', 'unityblocks' ),
				'sanitize_callback' => 'sanitize_text_field',
				'show_in_rest'      => true,
				'default'           => '',
			)
	...
	public function form_recaptcha_assets() {
		...
		if ( has_block( 'unityblocks/form' ) && ! empty( $recaptcha_site_key ) && ! empty( $recaptcha_secret_key ) ) {
			...
			wp_enqueue_script(
				'unityblocks-google-recaptcha',
				UnityBlocks()->asset_source( 'js' ) . 'unityblocks-google-recaptcha.js',
	...
	public function register_form_blocks() {
		...
		foreach ( $form_blocks as $form_block ) {
			...
			register_block_type(
				"unityblocks/field-${form_block}",
				array(
					'parent'          => array( 'unityblocks/form' ),
	...
	public function render_form( $atts, $content ) {
		...
		<div class="unityblocks-form" id="<?php echo esc_attr( $this->form_hash ); ?>">
			...
			<form action="<?php echo esc_url( sprintf( '%1$s#%2$s', set_url_scheme( get_the_permalink() ), $this->form_hash ) ); ?>" method="post">
				...
				if ( false === strpos( $content, 'unityblocks-form__submit' ) ) :
	...
	public function render_field_name( $atts ) {
		...
		$label            = isset( $atts['label'] ) ? $atts['label'] : __( 'Name', 'unityblocks' );
		...
		$label_first_name = isset( $atts['labelFirstName'] ) ? $atts['labelFirstName'] : __( 'First', 'unityblocks' );
		$label_last_name  = isset( $atts['labelLastName'] ) ? $atts['labelLastName'] : __( 'Last', 'unityblocks' );
		...
		?>
		<input type="hidden" id="name-field-id" name="name-field-id" class="unityblocks-name-field-id" value="field-<?php echo esc_attr( $label_slug ); ?>" />
		<?php

		if ( $has_last_name ) {

			$style   = implode( ' ', (array) apply_filters( 'unityblocks_render_label_color_wrapper_styles', array(), $atts ) );
			$styles  = empty( $style ) ? '' : " style='$style'";
			$classes = implode( ' ', (array) apply_filters( 'unityblocks_render_label_color_wrapper_class', array( 'unityblocks-form__subtext' ), $atts ) );
		...
		<input type="text" id="<?php echo esc_attr( $label_slug ); ?>" name="field-<?php echo esc_attr( $label_slug ); ?>[value]" class="unityblocks-field unityblocks-field--name" <?php echo esc_attr( $required_attr ); ?> />
	...
	public function render_field_email( $atts ) {
		...
		$label         = isset( $atts['label'] ) ? $atts['label'] : __( 'Email', 'unityblocks' );
		...
		<input type="hidden" id="email-field-id" name="email-field-id" class="unityblocks-email-field-id" value="field-<?php echo esc_attr( $label_slug ); ?>" />
		<input type="email" id="<?php echo esc_attr( $label_slug ); ?>" name="field-<?php echo esc_attr( $label_slug ); ?>[value]" class="unityblocks-field unityblocks-field--email" <?php echo esc_attr( $required_attr ); ?> />
	...
	public function render_field_textarea( $atts ) {
		...
		$label         = isset( $atts['label'] ) ? $atts['label'] : __( 'Message', 'unityblocks' );
		...
		<textarea name="field-<?php echo esc_attr( $label_slug ); ?>[value]" id="<?php echo esc_attr( $label_slug ); ?>" class="unityblocks-field unityblocks-textarea" rows="20" <?php echo esc_attr( $required_attr ); ?>></textarea>
	...
	public function render_field_text( $atts ) {
		...
		$label         = isset( $atts['label'] ) ? $atts['label'] : __( 'Text', 'unityblocks' );
		...
		<input type="text" name="field-<?php echo esc_attr( $label_slug ); ?>[value]" id="<?php echo esc_attr( $label_slug ); ?>" class="unityblocks-field unityblocks-text" <?php echo esc_attr( $required_attr ); ?>>
	...
	public function render_field_date( $atts ) {
		...
		wp_enqueue_script(
			'unityblocks-datepicker',
			UnityBlocks()->asset_source( 'js' ) . 'unityblocks-datepicker.js',
		...
		$label         = isset( $atts['label'] ) ? $atts['label'] : __( 'Date', 'unityblocks' );
		...
		<input type="text" id="<?php echo esc_attr( $label_slug ); ?>" name="field-<?php echo esc_attr( $label_slug ); ?>[value]" class="unityblocks-field unityblocks-field--date" <?php echo esc_attr( $required_attr ); ?> />
	...
	public function render_field_phone( $atts ) {
		...
		$label         = isset( $atts['label'] ) ? $atts['label'] : __( 'Phone', 'unityblocks' );
		...
		<input type="tel" id="<?php echo esc_attr( sanitize_title( $label ) ); ?>" name="field-<?php echo esc_attr( $label_slug ); ?>[value]" class="unityblocks-field unityblocks-field--telephone" <?php echo esc_attr( $required_attr ); ?> />
	...
	public function render_field_radio( $atts ) {
		...
		$label         = isset( $atts['label'] ) ? $atts['label'] : __( 'Choose one', 'unityblocks' );
		...
		print( '<div class="unityblocks-field">' );
		...
		foreach ( $the_options as $key => $value ) {
			...
			printf(
				...
				<label class="unityblocks-radio-label" for="%1$s">%5$s</label>',
	...
	public function render_field_select( $atts ) {
		...
		$label      = isset( $atts['label'] ) ? $atts['label'] : __( 'Select', 'unityblocks' );
		...
		printf(
			'<select class="select unityblocks-field" name="field-%1$s[value]">',
			esc_attr( $label_slug )
		);
	...
	public function render_field_checkbox( $atts ) {
		...
		if ( $required ) {
			...
			wp_enqueue_script(
				'unityblocks-checkbox-required',
				UnityBlocks()->asset_source( 'js' ) . 'unityblocks-checkbox-required.js',
				array(),
				UNITYBLOCKS_VERSION,
				true
			);
		...
		printf(
			'<div class="unityblocks-field checkbox%1$s">
				%2$s',
		...
		foreach ( $the_options as $value ) {
			...
			printf(
				...
				<label class="unityblocks-checkbox-label" for="%1$s">%4$s</label>',
				esc_attr( $label_slug . '-' . sanitize_title( $value ) ),
				esc_attr( $label_slug ),
				esc_attr( $value ),
				esc_html( $value )
	...
	public function render_field_website( $atts ) {
		...
		$label         = isset( $atts['label'] ) ? $atts['label'] : __( 'Website', 'unityblocks' );
		...
		<input type="url" id="<?php echo esc_attr( sanitize_title( $label ) ); ?>" name="field-<?php echo esc_attr( $label_slug ); ?>[value]" class="unityblocks-field unityblocks-field--website" <?php echo esc_attr( $required_attr ); ?> />
	...
	public function render_field_hidden( $atts ) {
		...
		$label      = isset( $atts['label'] ) ? $atts['label'] : __( 'Hidden', 'unityblocks' );
		...
		<input type="hidden" value="<?php echo esc_attr( $value ); ?>" id="<?php echo esc_attr( sanitize_title( $label ) ); ?>" name="field-<?php echo esc_attr( $label_slug ); ?>[value]" class="unityblocks-field unityblocks-field--hidden" />
	...
	public function render_field_label( $atts, $field_label, $count = 1 ) {
		...
		$styles     = implode( ' ', (array) apply_filters( 'unityblocks_render_label_color_wrapper_styles', array(), $atts ) );
		$classes    = implode( ' ', (array) apply_filters( 'unityblocks_render_label_color_wrapper_class', array( 'unityblocks-label' ), $atts ) );
		 ...
		 */
		$required_text  = (string) apply_filters( 'unityblocks_form_label_required_text', '&#42;', $field_label );
		$required_attr  = ( isset( $atts['required'] ) && $atts['required'] ) ? 'required' : '';
	...
	public function render_field_submit_button( $atts ) {
		...
		$btn_text             = isset( $atts['submitButtonText'] ) ? $atts['submitButtonText'] : __( 'Submit', 'unityblocks' );
		...
		<div class="unityblocks-form__submit wp-block-button">
			...
			<?php wp_nonce_field( 'unityblocks-form-submit', 'form-submit' ); ?>
			<input type="hidden" name="action" value="unityblocks-form-submit">
	...
	public function process_form_submission( $atts ) {
		 ...
		 */
		$honeypot_check = filter_input( INPUT_POST, 'unityblocks-verify-email', FILTER_SANITIZE_STRING );
		 ...
		 */
		do_action( 'unityblocks_before_form_submit', $_POST, $atts );

		/**
		 * Filter to disable the UnityBlocks form emails.
		 *
		 * @param bool false Whether or not the emails should be disabled.
		 */
		$send_email = (bool) apply_filters( 'unityblocks_form_email_send', true );
		...
		unset( $_POST['form-submit'], $_POST['_wp_http_referer'], $_POST['action'], $_POST['form-hash'], $_POST['unityblocks-verify-email'], $_POST['email-field-id'], $_POST['name-field-id'] );
		 ...
		 */
		$to = (string) apply_filters( 'unityblocks_form_email_to', $to, $_POST, $post_id );
		 ...
		 * @param array  $_POST   Submitted form data.
		 */
		$subject = (string) apply_filters( 'unityblocks_form_email_subject', $this->setup_email_subject( $atts, $email_field_id, $name_field_id ), $_POST );
		 ...
		 * @param integer $post_id             Current post ID.
		 */
		$email_content = (string) apply_filters( 'unityblocks_form_email_content', $this->email_content, $_POST, $post_id );

		$reply_to  = isset( $_POST[ $email_field_id ]['value'] ) ? esc_html( $_POST[ $email_field_id ]['value'] ) : esc_html( get_bloginfo( 'admin_email' ) );
		$from_name = ( isset( $_POST[ $name_field_id ]['value'] ) && ! empty( $_POST[ $name_field_id ]['value'] ) ) ? esc_html( $_POST[ $name_field_id ]['value'] ) . " <{$reply_to}>" : /* translators: 1. Reply to email address; eg: email@example.com */ sprintf( __( 'Form Submission <%s>', 'unityblocks' ), $reply_to );
		...
		$email_headers = (array) apply_filters(
			'unityblocks_form_email_headers',
			array(
				"From: {$from_name}",
				"Reply-To: {$reply_to}",
			),
		...
		if ( isset( $matches[1] ) ) {
			...
			array_walk(
				...
				function( $match, $key ) use ( $matches, &$subject, &$email_field_id, &$name_field_id ) {
					...
					if ( __( 'name', 'unityblocks' ) === $slug_match ) {
					 ...
					 */
					$replacement = (string) apply_filters( 'unityblocks_form_shortcode_match', $value, $slug_match );
	...
	public function success_message( $atts ) {
		...
		$success_message = (string) apply_filters(
			'unityblocks_form_success_message',
			sprintf(
				'<div class="unityblocks-form__submitted">%1$s %2$s</div>',
	...
	private function verify_recaptcha( $recaptcha_token ) {
		...
		$verify_token_request = wp_remote_post(
			...
			array(
				'timeout' => 30,
				'body'    => array(
					'secret'   => get_option( 'unityblocks_google_recaptcha_secret_key' ),
