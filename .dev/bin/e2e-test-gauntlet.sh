...
$WP_CLI db drop --yes
$WP_CLI db create
$WP_CLI core install --url="http://localhost:8080" --title="UnityBlocks" --admin_user="admin" --admin_password="password" --admin_email="test@admin.com" --skip-email
$WP_CLI site empty --uploads --yes
(curl -N http://loripsum.net/api/5 | $WP_CLI post generate --post_content --count=25) &> /dev/null

echo "Activating UnityBlocks."
cp -r build/unityblocks wordpress_$WP_VERSION/wp-content/plugins/unityblocks
$WP_CLI plugin activate unityblocks
