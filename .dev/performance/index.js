...
async function setUpGitBranch(branch, environmentDirectory) {
	...
	await runShellScript("yarn && yarn build", environmentDirectory);
}
...
async function runPerformanceTests(branches, options) {
	...
	// Clone in UnityBlocks and prepare repo for use with testing.
	const baseDirectory = await git.clone(config.gitRepositoryURL);
	await runShellScript(`mv ${baseDirectory} unityblocks`);
	...
	for (const branch of branches) {
		...
		await runShellScript(
			`./vendor/bin/wp core download --path=${environmentDirectory}`
		);
		await runShellScript(
			`./vendor/bin/wp config create --dbhost=127.0.0.1 --dbname=unityblocks${index} --dbuser=root --dbpass='' --path=${environmentDirectory}`
		);
		await runShellScript(
			`./vendor/bin/wp db create --path=${environmentDirectory}`
		);
		await runShellScript(
			`./vendor/bin/wp core install --url="http://localhost:8889" --title=UnityBlocks --admin_user=admin --admin_password=password --admin_email=test@admin.com --skip-email --path=${environmentDirectory}`
		);
		await runShellScript(
			`./vendor/bin/wp post generate --count=5 --path=${environmentDirectory}`
		);
		await runShellScript(
			`./vendor/bin/wp theme install go --activate --path=${environmentDirectory}`
		);

		// 2- Clone UnityBlocks repo into environment directory.
		log("\n>> Preparing the tests directory");
		log("    >> Branch: " + branch);
		log("    >> Cloning the repository");

		await runShellScript(
			`cp -R unityblocks ${environmentDirectory}/wp-content/plugins`
		);
		await setUpGitBranch(
			branch,
			`${environmentDirectory}/wp-content/plugins/unityblocks`
		);
		await runShellScript(
			`./vendor/bin/wp plugin activate unityblocks --path=${environmentDirectory}`
		);
		index++;
	}
	...
	for (const branch of branches) {
		...
		log(
			"\n>> Perf Tests Directory : " +
				formats.success(
					`${branchDirectories[branch]}/wp-content/plugins/unityblocks`
				)
		);
		log(
	...
	for (const testSuite of testSuites) {
		...
		for (let i = 0; i < 3; i++) {
			...
			for (const branch of branches) {
				...
				await runShellScript(
					'sudo kill $(ps ax | pgrep -f "server") > /dev/null 2>&1 &'
				);
			}
		}
			...
			const medians = mapValues(
				{
					...
					inserterOpen: rawResults.map((r) => r[branch].inserterOpen),
					minInserterOpen: rawResults.map((r) => r[branch].minInserterOpen),
					maxInserterOpen: rawResults.map((r) => r[branch].maxInserterOpen),
					inserterSearch: rawResults.map((r) => r[branch].inserterSearch),
					minInserterSearch: rawResults.map((r) => r[branch].minInserterSearch),
					maxInserterSearch: rawResults.map((r) => r[branch].maxInserterSearch),
					inserterHover: rawResults.map((r) => r[branch].inserterHover),
					minInserterHover: rawResults.map((r) => r[branch].minInserterHover),
					maxInserterHover: rawResults.map((r) => r[branch].maxInserterHover),
				},
				median
			);
	...
	for (const testSuite of testSuites) {
		...
		Object.entries(results[testSuite]).reduce((acc, [key, val]) => {
