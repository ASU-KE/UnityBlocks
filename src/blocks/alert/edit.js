...
const Edit = ( props ) => {
	...
	const { textAlign, title, value } = attributes;

	const prevClassName = usePrevious( className );

	useEffect( () => {
		// Convert is-{type}-alert to is-style-{type}.
		// See: https://github.com/godaddy-wordpress/unityblocks/pull/781
			...
			>
				{ ( ! RichText.isEmpty( title ) || isSelected ) && (
					<RichText
						...
						placeholder={ __( 'Write title…', 'unityblocks' ) }
						value={ title }
						className="wp-block-unityblocks-alert__title"
						onChange={ ( newTitle ) =>
				...
				<RichText
					/* translators: placeholder text for input box */
					placeholder={ __( 'Write text…', 'unityblocks' ) }
					value={ value }
					className="wp-block-unityblocks-alert__text"
					onChange={ ( newValue ) =>
