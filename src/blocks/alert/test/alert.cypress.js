...
describe( 'Test UnityBlocks Alert Block', function () {
	/**
	...
	it( 'Test alert block saves with empty values.', function () {
		helpers.addBlockToPost( 'unityblocks/alert', true );

		helpers.savePage();

		helpers.checkForBlockErrors( 'unityblocks/alert' );

		helpers.viewPage();

		cy.get( '.wp-block-unityblocks-alert' ).should( 'be.empty' );
	...
	it( 'Test alert block saves and displays properly.', function () {
		...
		cy.get( '.wp-block-unityblocks-alert__title' )
			.first()
			.focus()
			.type( 'Test Title' );

		cy.get( '.wp-block-unityblocks-alert__text' )
			.first()
			.focus()
			.type( 'Test text' );
	...
	it( 'Test alert style classes are applied in the editor.', function () {
		...
		cy.get( '[data-type="unityblocks/alert"]' ).first().click( 'right' );
		...
		cy.get( '.wp-block-unityblocks-alert' ).should(
			'have.class',
			'is-style-info'
		);
