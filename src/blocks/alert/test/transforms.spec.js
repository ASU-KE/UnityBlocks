...
describe( 'unityblocks/alert transforms', () => {
	beforeAll( () => {
		// Register the block.
	...
	it( 'should transform from core/paragraph block', () => {
		const coreParagraph = createBlock( 'core/paragraph', {
	...
	it( 'should transform raw html to block', () => {
		...
		const HTML = `<div class="wp-block-unityblocks-alert">
				<p class="wp-block-unityblocks-alert__title">${ title }</p>
				<p class="wp-block-unityblocks-alert__text">${ value }</p>
			</div>`;
