 ...
 * @type {string[]}
 */
const ALLOWED_BLOCKS = [ 'unityblocks/accordion-item' ];
...
const getCount = memoize( ( count ) => {
	return times( count, () => [ 'unityblocks/accordion-item' ] );
} );
...
const AccordionEdit = ( props ) => {
	const { clientId, attributes, className } = props;
	...
	return (
		<Fragment>
			{ isBlockSelected && (
				<Inspector insertBlock={ insertBlock } { ...props } />
			) }
			<div className={ className }>
				...
				/>
				{ isBlockSelected && (
					<div className="unityblocks-block-appender">
						<Tooltip
							text={ __( 'Add accordion item', 'unityblocks' ) }
						>
							<Button
								label={ __(
									'Add accordion item',
