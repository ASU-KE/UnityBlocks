 ...
 */
const TEMPLATE = [
	[ 'core/paragraph', { placeholder: __( 'Add content…', 'unityblocks' ) } ],
];
...
const AccordionItemEdit = ( props ) => {
	...
	const { isEditing } = useSelect( ( select ) => {
		...
		const hasSelectedChildren = getBlocks( props.clientId ).filter(
			( elem ) =>
				elem.clientId === getSelectedBlockClientId() ||
				elem.clientId ===
		...
		return {
			isEditing:
				getSelectedBlockClientId() === props.clientId ||
			...
			>
				<RichText
					...
					className={ classnames(
						'wp-block-unityblocks-accordion-item__title',
						{
				...
				{ ( isEditing === true || attributes.open ) && (
					<div
						className="wp-block-unityblocks-accordion-item__content"
...
export default compose( [ applyWithColors ] )( AccordionItemEdit );
