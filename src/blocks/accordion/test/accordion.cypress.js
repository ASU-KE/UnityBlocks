/**
 * Include our constants
 */
import * as helpers from '../../../../.dev/tests/cypress/helpers';

describe( 'Block: Accordion', () => {
	beforeEach( () => {
		helpers.addBlockToPost( 'unityblocks/accordion', true );
	} );

	/**
	 * Test that we can add an accordion item to the content, not add any text or
	 * alter any settings, and are able to successfully save the block without errors.
	 */
	it( 'can be inserted without errors', () => {
		cy.get( '[data-type="unityblocks/accordion"]' ).should( 'exist' );
		helpers.checkForBlockErrors( 'unityblocks/accordion' );
	} );

	/**
	 * Test that we can add an accordion item to the page, add content to it,
	 * save and it displays properly without errors.
	 */
	it( 'can be modified without errors', () => {
		cy.get(
			'[data-type="unityblocks/accordion"] .wp-block-unityblocks-accordion-item__title'
		).type( 'title' );
		helpers.checkForBlockErrors( 'unityblocks/accordion' );
	} );

	/**
	 * Test 'Display Open' attribute works
	 */
	it( 'can be expanded by default when toggling the "open" attribute', () => {
		cy.get( '.editor-post-title__input' ).click();
		cy.get(
			'[data-type="unityblocks/accordion"] .wp-block-unityblocks-accordion-item__content'
		).should( 'not.exist' );

		cy.get( '[data-type="unityblocks/accordion-item"]' ).click();
		helpers.toggleSettingCheckbox( 'Display as open' );

		cy.get( '.editor-post-title__input' ).click();
		cy.get(
			'[data-type="unityblocks/accordion"] .wp-block-unityblocks-accordion-item__content'
		).should( 'exist' );

		helpers.checkForBlockErrors( 'unityblocks/accordion' );
	} );

	/**
	 * Test that multiple accordion items display as expected
	 */
	it( 'can add multiple accordion item blocks', () => {
		cy.get( '[data-type="unityblocks/accordion"]' ).click( 'top', {
			force: true,
		} );

		cy.get( '.unityblocks-block-appender button' ).click();

		cy.get( '[data-type="unityblocks/accordion"]' )
			.find( '[data-type="unityblocks/accordion-item"]' )
			.should( 'have.length', 2 );

		helpers.checkForBlockErrors( 'unityblocks/accordion' );
	} );

	/**
	 * Test the accordion block color settings
	 */
	it( 'can apply color settings', () => {
		cy.get(
			'[data-type="unityblocks/accordion-item"] .wp-block-unityblocks-accordion-item__title'
		).type( 'Accordion title' );

		// Title - Background color
		helpers.setColorSetting( 'background', '#000000' );
		cy.get(
			'[data-type="unityblocks/accordion-item"] .wp-block-unityblocks-accordion-item__title'
		).should( 'have.css', 'background-color', `rgb(0, 0, 0)` );

		// Title - Text color
		helpers.setColorSetting( 'text', '#FFFFFF' );
		cy.get(
			'[data-type="unityblocks/accordion-item"] .wp-block-unityblocks-accordion-item__title'
		).should( 'have.css', 'color', `rgb(255, 255, 255)` );

		cy.get(
			'[data-type="unityblocks/accordion-item"] .wp-block-unityblocks-accordion-item__content p'
		)
			.click()
			.type( 'Content' );

		// Content - Background color
		helpers.setColorSetting( 'background', '#000000' );
		cy.get(
			'[data-type="unityblocks/accordion-item"] .wp-block-unityblocks-accordion-item__content p'
		).should( 'have.css', 'background-color', `rgb(0, 0, 0)` );

		// Content - Text color
		helpers.setColorSetting( 'text', '#FFFFFF' );
		cy.get(
			'[data-type="unityblocks/accordion-item"] .wp-block-unityblocks-accordion-item__content p'
		).should( 'have.css', 'color', `rgb(255, 255, 255)` );

		helpers.checkForBlockErrors( 'unityblocks/accordion' );
	} );

	/**
	 * Test the accordion block custom classes
	 */
	it( 'can have custom classes', () => {
		cy.get( '[data-type="unityblocks/accordion-item"]' ).first().click();
		helpers.addCustomBlockClass( 'my-custom-class', 'accordion-item' );

		helpers.checkForBlockErrors( 'unityblocks/accordion' );

		cy.get( '.wp-block-unityblocks-accordion-item' ).should(
			'have.class',
			'my-custom-class'
		);
	} );
} );
