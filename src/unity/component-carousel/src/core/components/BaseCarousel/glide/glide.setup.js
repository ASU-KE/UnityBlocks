...
function setupCaroarousel( {
	...
	const sliderConfig = buildConfig(
		perView,
		isFullWidth,
		hasPeek,
		isDraggable
	...
	slider.on( 'move', () => {
		...
		const imageGalleryNav = gliderElement.querySelector(
			'.navigation-slider'
		);
		if ( imageGalleryNav ) {
			...
			const imageNav = gliderElement.querySelector(
				'.image-navigator-images'
			);
			...
			if (
				...
				if ( currentIndex > 1 ) {
					const pos =
						halfDiff -
						inactiveImage.offsetWidth * ( currentIndex - 1 );
