...
export const LayoutSelectorResults = ( { layouts, category, onInsert } ) => {
	...
	const imageCategory =
		getEntityRecord( 'root', 'site' )?.image_category || '';
		...
		() =>
			layouts
				...
				.map( ( layout ) => ( {
					 ...
					 */
					blocks: applyFilters(
						'unityblocks.layoutPreviewBlocks',
						layout.parsedBlocks
					),
				} ) ),
	...
	return category && !! filteredLayouts.length ? (
		<div className="unityblocks-layout-selector__layouts">
			<LayoutPreviewList
...
export const LayoutPreview = ( { layout, onClick } ) => {
	...
	return (
		<Button
			className={ classnames( 'unityblocks-layout-selector__layout' ) }
...
export const LayoutPreviewPlaceholder = () => {
	return (
		<div className="unityblocks-layout-selector__layout is-placeholder">
			<Spinner />
		</div>
	);
};
 ...
 */
export const LayoutPreviewList = ( {
	layouts,
	shownLayouts,
	onClickLayout,
} ) => {
