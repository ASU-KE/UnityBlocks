...
const DEFAULT_STATE = {
	...
	layouts: unityblocksLayoutSelector.layouts || [],
	computedLayouts: [],
	categories: unityblocksLayoutSelector.categories || [],
...
const actions = {
	...
	incrementLayoutUsage: ( layout ) => ( {
		type: 'INCREMENT_LAYOUT_USAGE',
		layout,
		time: Date.now(),
	} ),
};

const store = registerStore( 'unityblocks/template-selector', {
	...
	selectors: {
		...
		getLayouts: ( state ) => {
			...
			return layouts.map( ( layout ) => {
				const { time, count = 0 } =
					state.layoutUsage[ kebabCase( layout.label ) ] || {};
				return {
					...layout,
	...
	resolvers: {
		*isTemplateSelectorActive() {
			const isCleanNewPost = yield select(
				'core/editor',
				'isCleanNewPost'
			);
