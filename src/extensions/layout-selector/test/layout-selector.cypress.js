...
describe( 'Extension: Layout Selector', () => {
	beforeEach( () => {
		...
		cy.get( '.unityblocks-layout-selector-modal' ).should( 'exist' );
	} );

	it( 'loads layouts of each category', () => {
		// Click "Test One" category.
		cy.get( '.unityblocks-layout-selector__sidebar__item:nth-child(1)' )
			.find( 'a' )
			.click();
		cy.get(
			'.unityblocks-layout-selector__layouts .unityblocks-layout-selector__layout'
		).should( 'not.have.length', 0 );

		// Click "Test Two" category.
		cy.get( '.unityblocks-layout-selector__sidebar__item:nth-child(2)' )
			.find( 'a' )
			.click();
		cy.get(
			'.unityblocks-layout-selector__layouts .unityblocks-layout-selector__layout'
	...
	it( 'inserts layout into page', () => {
		...
		cy.get( '.unityblocks-layout-selector__layout' ).first().click();
	...
	it( 'inserts blank layout into page', () => {
		...
		cy.get( '.unityblocks-layout-selector__add-button' ).first().click();

		// Post title block should be empty.
		cy.get( '.editor-post-title__block' )
			.find( 'textarea' )
			.should( 'be.empty' );
