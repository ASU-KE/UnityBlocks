...
describe( 'layout-selector-results', () => {
	describe( 'LayoutPreviewList', () => {
		...
		const layouts = [
			{
				...
				parsedBlocks: [
					[ 'core/paragraph', { content: 'layout-one' }, [] ],
				],
			},
			{
				label: 'layout-two',
				...
				parsedBlocks: [
					[ 'core/paragraph', { content: 'layout-two' }, [] ],
				],
			},
			{
				label: 'layout-three',
				category: 'about',
				blocks: [
					[ 'core/paragraph', { content: 'layout-three' }, [] ],
				],
				parsedBlocks: [
					[ 'core/paragraph', { content: 'layout-three' }, [] ],
				],
		...
		it( 'renders shown layouts', () => {
			...
			expect( wrapper.find( 'LayoutPreview' ) ).toHaveLength(
				layouts.length
			);
			expect( wrapper.find( 'LayoutPreviewPlaceholder' ) ).toHaveLength(
				0
			);
		} );
	...
	describe( 'LayoutPreviewPlaceholder', () => {
		...
		it( 'renders', () => {
			expect(
				wrapper.exists( '.unityblocks-layout-selector__layout' )
			).toEqual( true );
		} );
	} );
	...
	describe( 'LayoutPreview', () => {
		...
		it( 'should call onClick() on Button click', () => {
			wrapper
				.find( '.unityblocks-layout-selector__layout' )
				.invoke( 'onClick' )();
	...
	describe( 'LayoutSelectorResults', () => {
		...
		it( 'renders', () => {
			expect(
				wrapper.exists( '.unityblocks-layout-selector__layouts' )
			).toEqual( true );
		} );
		...
		it( 'renders message when no layouts are available for the selected category', () => {
			...
			expect(
				wrapper.find( '.unityblocks-layout-selector__layout' )
			).toHaveLength( 0 );
			expect( wrapper.text() ).toContain(
				'No layouts are available for this category.'
			);
		} );
	} );
	...
	describe( 'sanitizeBlocks()', () => {
		...
		it( 'sanitize blocks without breaking them', () => {
			const blocks = [
				createBlock( 'unityblocks/gallery-stacked' ),
				createBlock( 'unityblocks/gallery-masonry' ),
				{
					...
					attributes: {
						images: [
							{
								url:
									'https://wpnux.godaddy.com/v2/api/image?aspect=3%3A4&index=2&lang=en_US&seed=wpnux_layout_home-1&size=large&category=fashion',
								link:
									'https://wpnux.godaddy.com/v2/api/image?aspect=3%3A4&index=2&lang=en_US&seed=wpnux_layout_home-1&size=large&category=fashion',
							...
							{
								url:
									'https://wpnux.godaddy.com/v2/api/image?aspect=3%3A4&index=3&lang=en_US&seed=wpnux_layout_home-1&size=large&category=fashion',
								link:
									'https://wpnux.godaddy.com/v2/api/image?aspect=3%3A4&index=3&lang=en_US&seed=wpnux_layout_home-1&size=large&category=fashion',
								alt: 'Image Description',
								id: '3',
			...
			expect(
				parsedBlocks
					...
					.map( helpers.filterBlockObjectResult )
			).toEqual( [] );
		} );
	} );
} );
