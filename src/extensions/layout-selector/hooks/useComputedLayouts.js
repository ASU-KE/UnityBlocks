...
import {
	requestIdleCallback,
	cancelIdleCallback,
} from '../../../utils/background-task-api';
...
const getBlocksFromTemplate = ( name, attributes, innerBlocks = [] ) => {
	return createBlock(
		...
		innerBlocks &&
			innerBlocks.map(
				( [ blockName, blockAttributes, blockInnerBlocks ] ) =>
...
function useComputedLayouts() {
	const layouts = useSelect(
		( select ) => select( 'unityblocks/template-selector' ).getLayouts(),
		[]
	);
	const computedLayoutsStore = useSelect(
		( select ) =>
			select( 'unityblocks/template-selector' ).getComputedLayouts(),
		[]
	);
	const selectedCategory = useSelect(
		( select ) =>
			select( 'unityblocks/template-selector' ).getSelectedCategory(),
		[]
	);
	const { updateComputedLayouts } = useDispatch(
		'unityblocks/template-selector'
	);
	...
	const parseLayout = ( layout ) => ( {
		...layout,
		parsedBlocks: parseBlocks( layout ),
	} );
	...
	useEffect( () => {
		...
		layoutsQueue = [ ...mostUsedLayouts, ...layouts ];

		computedCategories = [ selectedCategory ];
		taskQueueId = requestIdleCallback( processLayoutsQueue, {
			timeout: 1000,
		} );
