...
describe( '#Pagination', () => {
	...
	it( "should 'active' selected item", () => {
		...
		fireEvent.click( clickableElement );
		setTimeout( () =>
			expect( clickableElement.parentElement.className ).toContain(
				'active'
			)
		);
	} );
...
describe( '#Pagination variations', () => {
	...
	it( 'should set active currentPage', () => {
		...
		const component = renderPagination( props );
		const paginationItem = component.queryAllByTestId( 'page-link' )[
			currentPage
		].parentElement;
