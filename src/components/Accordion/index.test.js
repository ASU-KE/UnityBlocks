...
const renderAccordion = ( args ) => {
	return render(
		<Accordion cards={ args.cards } openedCard={ args.openedCard } />
	);
};
...
describe( '#Accordion opened', () => {
	it( 'should define the component opened', async () => {
		...
		const componentActioner = await component.findByTestId(
			'accordion-opener'
		);

		expect( component ).toBeDefined();
		expect( componentActioner.getAttribute( 'aria-expanded' ) ).toBe(
			'true'
		);
	} );
} );
