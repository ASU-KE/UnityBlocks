/* eslint-disable react/jsx-props-no-spreading, jsdoc/valid-types, jsdoc/check-tag-names, jsdoc/no-undefined-types */
// @ts-check
...
const defaultArgs = {
	...
	title: 'Default title',
	body:
		"<span style='font-weight: bold;'>(Bold!) Body copy goes here.</span> Limit to 5 lines max. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua eiusmod tempo.",
	buttons: [
		{
			color: 'maroon',
			size: 'default',
...
describe( '#Card options', () => {
	...
	describe( '#Icon card', () => {
		...
		it( 'should render icon element', () => {
			expect(
				component.queryByTestId( 'card-icon' )
			).toBeInTheDocument();
		} );
	} );
	const types = [
	...
	test.each( types )(
		'should define %p type of card',
		( type, className ) => {
			const customProps = { ...defaultArgs, type };
			component = renderCard( customProps );
			expect(
				component.queryByTestId( 'card-container' ).className
			).toContain( className );
		}
	);
} );
